# eegfist
This is an attempt to create a classifier for the data provided at https://www.physionet.org/pn4/eegmmidb/. This data yields 64 channels of EEG data while performing various tasks. The following tasks have been addressed (description taken from the project's website):
...
> A target appears on either the top or the bottom of the screen. The subject opens and closes either both fists (if the target is on top) "..." until the target disappears. Then the subject relaxes.
...
- current feature vectors yield a classification rate of 69% when T1_left and T1_right are combined in one single class "fist closed" and compared to the feature vector T3 (relaxed)
- current feature vectors yield non-usable results when run on class T1_left vs T1_right (roughly 51%)
- next step would be to try to improve the feature vectors
...
## Long summary
 ...
 - matlab has been chosen for signal processing as it provides easy access to a lot of useful libraries
 ...
 - __third approach__
  ...
  - No better classification rate than 50-51% could be achieved when classifying T1_left vs T1_right by completely removing T0 from the data
    - the feature vectors obv aren"t suitable for classification between these two classes, explaining the very bad results during the first approaches
